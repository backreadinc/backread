...
// ─── LAYOUTS with working build() functions ───────────────────────────────────
const LAYOUTS = [
  {
  ...
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      ...
      bx({l:40,t:Math.round(H*.25)+fs(W,44)*2+72,w:120,h:38,fill:'#4f46e5',rx:8}),
      tx('Learn more',{l:56,t:Math.round(H*.25)+fs(W,44)*2+83,w:88,fs:12,fw:'700',fill:'#ffffff',ta:'center'}),
    ]),
  },
  {
  ...
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      ...
      bx({l:0,t:H-4,w:W,h:4,fill:'#0f172a'}),
      tx('MAGAZINE',{l:40,t:16,w:200,fs:fs(W,11),fw:'800',fill:'#0f172a',ff:'JetBrains Mono'}),
      tx('ISSUE 01 · 2025',{l:W-200,t:16,w:160,fs:fs(W,10),fw:'600',fill:'#94a3b8',ta:'right',ff:'JetBrains Mono'}),
      bx({l:40,t:50,w:Math.round(W*.55),h:Math.round(H*.65),fill:'#e2e8f0',rx:4}),
      tx('Feature Story Title\nThat Spans Lines',{l:Math.round(W*.6),t:60,w:Math.round(W*.33),fs:fs(W,30),fw:'800',fill:'#0f172a',ff:'Jost',lh:1.08}),
      tx('A compelling opening paragraph that draws the reader into the story.',{l:Math.round(W*.6),t:60+fs(W,30)*2+20,w:Math.round(W*.33),fs:fs(W,13),fill:'#475569',lh:1.7}),
    ]),
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      ...
      bx({l:50,t:48+fs(W,30)+12,w:48,h:3,fill:'#10b981',rx:2}),
      ...(['$0M','ARR','#ecfdf5','#10b981'] as any[]).map(()=>null).filter(()=>false),
  ...
  },
  {
    ...
    build:(W:number,H:number)=>pg('#f8fafc',[
      bx({l:0,t:0,w:4,h:H,fill:'#4f46e5'}),
      tx('Meet the Team',{l:40,t:50,w:W-80,fs:fs(W,34),fw:'800',fill:'#0f172a'}),
      bx({l:40,t:50+fs(W,34)+12,w:48,h:3,fill:'#4f46e5',rx:2}),
      ...[['CEO','Alex Chen'],['CTO','Sam Rivera'],['Design','Jordan Kim'],['Growth','Taylor Park']].flatMap(([role,name]:string[],i:number)=>{
        const cw=Math.round((W-100)/4), cx=40+i*(cw+13)
        return [
          bx({l:cx,t:Math.round(H*.38),w:cw,h:Math.round(H*.48),fill:'#ffffff',rx:14}),
          bx({l:cx+Math.round(cw/2)-30,t:Math.round(H*.38)+20,w:60,h:60,fill:'#e0e7ff',rx:30}),
          tx(name,{l:cx+10,t:Math.round(H*.38)+96,w:cw-20,fs:fs(W,14),fw:'700',fill:'#0f172a',ta:'center'}),
          tx(role,{l:cx+10,t:Math.round(H*.38)+96+fs(W,14)+6,w:cw-20,fs:10,fw:'600',fill:'#4f46e5',ta:'center',ff:'JetBrains Mono'}),
        ]
      }),
    ]),
  },
  {
    ...
    build:(W:number,H:number)=>pg('#0a0a0f',[
      ...
      tx('01',{l:W-80,t:H-44,w:60,fs:fs(W,11),fw:'700',fill:'rgba(255,255,255,.2)',ta:'right',ff:'JetBrains Mono'}),
    ]),
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      ...
      ...[['Feature One','#4f46e5'],['Feature Two','#10b981'],['Feature Three','#f59e0b']].flatMap(([title,col]:string[],i:number)=>{
        ...
        return [
          ...
          bx({l:cx+18,t:Math.round(H*.34)+20,w:36,h:36,fill:col,rx:8}),
          tx(title,{l:cx+18,t:Math.round(H*.34)+72,w:cw-36,fs:fs(W,16),fw:'700',fill:'#0f172a'}),
          tx('Describe this feature and the specific value it delivers to your audience.',{l:cx+18,t:Math.round(H*.34)+72+fs(W,16)+10,w:cw-36,fs:12,fill:'#64748b',lh:1.6}),
        ]
      }),
    ]),
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      bx({l:0,t:0,w:4,h:H,fill:'#4f46e5'}),
      tx('Project Timeline',{l:50,t:44,w:W-100,fs:fs(W,28),fw:'700',fill:'#0f172a'}),
      bx({l:Math.round(W*.5)-1,t:Math.round(H*.28),w:2,h:Math.round(H*.58),fill:'#e2e8f0'}),
      ...[['Phase 1','Discovery','Weeks 1–2'],['Phase 2','Design','Weeks 3–5'],['Phase 3','Build','Weeks 6–9'],['Phase 4','Launch','Week 10']].flatMap(([phase,title,time]:string[],i:number)=>{
        const y=Math.round(H*.28)+i*Math.round(H*.14)
        const left=i%2===0
        return [
          bx({l:Math.round(W*.5)-7,t:y+4,w:14,h:14,fill:'#4f46e5',rx:7}),
          tx(`${phase} — ${title}`,{l:left?Math.round(W*.5)+24:50,t:y,w:Math.round(W*.36),fs:fs(W,14),fw:'700',fill:'#0f172a',ta:left?'left':'right'}),
          tx(time,{l:left?Math.round(W*.5)+24:50,t:y+fs(W,14)+6,w:Math.round(W*.36),fs:10,fill:'#94a3b8',ff:'JetBrains Mono',ta:left?'left':'right'}),
        ]
      }),
    ]),
  },
  {
    ...
    build:(W:number,H:number)=>pg('#ffffff',[
      ...
      tx('INVOICE',{l:50,t:38,w:300,fs:fs(W,30),fw:'800',fill:'#0f172a',ff:'Jost'}),
      tx('#INV-0001',{l:50,t:38+fs(W,30)+8,w:200,fs:12,fill:'#4f46e5',ff:'JetBrains Mono'}),
      tx('Due: DD/MM/YYYY',{l:W-240,t:38,w:190,fs:12,fill:'#64748b',ta:'right',ff:'JetBrains Mono'}),
      bx({l:50,t:130,w:W-100,h:1,fill:'#e2e8f0'}),
      tx('Bill To',{l:50,t:148,w:200,fs:9,fw:'700',fill:'#94a3b8',ff:'JetBrains Mono'}),
      tx('Client Company Name\nclient@email.com',{l:50,t:164,w:280,fs:13,fill:'#0f172a',lh:1.7}),
      bx({l:50,t:220,w:W-100,h:36,fill:'#0f172a',rx:7}),
      tx('Description',{l:66,t:229,w:Math.round((W-100)*.5),fs:11,fw:'700',fill:'#ffffff'}),
      tx('Amount',{l:W-140,t:229,w:90,fs:11,fw:'700',fill:'#ffffff',ta:'right'}),
      ...[0,1,2].flatMap((i:number)=>[
        bx({l:50,t:256+i*44,w:W-100,h:44,fill:i%2===0?'#f8fafc':'#ffffff'}),
        tx('Service description',{l:66,t:267+i*44,w:Math.round((W-100)*.6),fs:12,fill:'#0f172a'}),
        tx('$0,000.00',{l:W-140,t:267+i*44,w:90,fs:12,fill:'#0f172a',ta:'right',fw:'600',ff:'JetBrains Mono'}),
      ]),
      tx('Total Due',{l:W-240,t:256+3*44+16,w:190,fs:13,fw:'700',fill:'#0f172a',ta:'right'}),
      tx('$0,000.00',{l:W-240,t:256+3*44+36,w:190,fs:fs(W,24),fw:'800',fill:'#4f46e5',ta:'right',ff:'Jost'}),
    ]),
  },
]
...
function proposal(W:number,H:number):any[] {
  ...
  return [
    ...
    pg('#ffffff',[
      ...
      tx('Describe the context, the core challenge, and what makes this proposal the right solution.',{l:50,t:52+fs(W,28)+30,fs:fs(W,14),fill:'#475569',w:W-100,lh:1.7}),
...
function caseStudy(W:number,H:number):any[] {
  ...
  return [
    ...
    pg('#ffffff',[
      ...
      tx("Describe the client's core problem in vivid, specific terms. What was happening, what was at stake?",{l:50,t:52+fs(W,28)+24,fs:fs(W,15),fill:'#475569',w:W-100,lh:1.7}),
...
export default function DocumentEditorPage({ params }: { params: { id: string } }) {
  ...
  // FIX: real thumbnail capture per page
  ...
  async function loadDocument(){
    ...
    if(cd?.pages?.length){
      ...
      // load first page once fabric is ready
  ...
  // FIX: proper Fabric init — no more setInterval polling
  function initFabric(){
    if(fabricReady.current||!canvasEl.current)return
    if(!(window as any).fabric){
      setTimeout(initFabric, 80)
      return
    }
    ...
    // if pages already loaded, render first page
    if(pagesRef.current.length>0){
      fc.setWidth(cWRef.current)
      fc.setHeight(cHRef.current)
      fc.loadFromJSON(pagesRef.current[0],()=>fc.renderAll())
    }
  }
  ...
  function waitForFabricThenLoad(pageJson:any, w:number, h:number){
    const attempt=()=>{
      if(fabricRef.current){
        ...
        fabricRef.current.loadFromJSON(pageJson,()=>fabricRef.current.renderAll())
      } else {
  ...
  // FIX: capture real thumbnail of current page
  function captureThumbnail(idx:number){
    if(!fabricRef.current)return
    try{
      const url=fabricRef.current.toDataURL({format:'jpeg',quality:0.5,multiplier:0.15})
      setThumbnails(prev=>({...prev,[idx]:url}))
    } catch(e){ /* cross-origin images may block toDataURL */ }
  }
  ...
  function addPage(){
    ...
    upd.splice(ni,0,blank)
    pagesRef.current=upd
    setPages([...upd])
    setCurrentPage(ni)
    currentPageRef.current=ni
  ...
  // FIX: Template — properly saves pages and loads canvas
  function applyTemplate(id:string,sizeId:string){
    const size=CANVAS_SIZES.find(s=>s.id===sizeId)||CANVAS_SIZES[0]
    setCanvasW(size.w);setCanvasH(size.h)
    cWRef.current=size.w;cHRef.current=size.h
    ...
    setShowTplModal(false)
    setThumbnails({})
    waitForFabricThenLoad(built[0], size.w, size.h)
  }
  ...
  function addTable(rows=4,cols=3){
    const fab=(window as any).fabric;const fc=fabricRef.current;if(!fc||!fab)return
    const cw=170,rh=42,x=100,y=100
    for(let i=0;i<rows;i++){
      for(let j=0;j<cols;j++){
        const isH=i===0
        fc.add(new fab.Rect({
          left:x+j*cw, top:y+i*rh, width:cw, height:rh,
          fill:isH?'#0f172a':i%2===0?'#f8fafc':'#ffffff',
          stroke:'#e2e8f0', strokeWidth:1, selectable:true,
        }))
        fc.add(new fab.IText(isH?`Column ${j+1}`:`Cell ${i},${j+1}`,{
          left:x+j*cw+8, top:y+i*rh+11, width:cw-16,
          fontSize:12, fontFamily:'Jost',
          fill:isH?'#ffffff':'#374151',
          fontWeight:isH?'600':'400', editable:true, selectable:true,
        }))
      }
    }
    fc.renderAll()
  ...
  async function exportPDF(){
    ...
    const pdf=new jsPDF({
      orientation:cWRef.current>cHRef.current?'landscape':'portrait',
      unit:'px', format:[cWRef.current,cHRef.current],
    })
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#f0f0f2',fontFamily:"'Jost',system-ui,sans-serif",color:'#0f172a'}}>
      ...
      {/* ── TOOLBAR ──────────────────────────────────────────────────────────── */}
      <div style={{height:46,background:'white',borderBottom:'1px solid #e5e7eb',display:'flex',alignItems:'center',padding:'0 10px',gap:2,flexShrink:0,overflowX:'auto'}}>
        {([
          ...
          {id:'text',  tip:'Text',  icon:<svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M2 4h11M7.5 4v8M4.5 12h6" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>},
          {id:'rect',  tip:'Rect',  icon:<svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="2" y="3" width="11" height="9" rx="2" stroke="currentColor" strokeWidth="1.4"/></svg>},
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          {([
            {l:'A',  v:fontColor, s:(v:string)=>{setFontColor(v);updateProp('fill',v)}},
            {l:'Fill',v:fillColor,s:(v:string)=>{setFillColor(v);updateProp('fill',v)}},
            {l:'BG', v:bgColor,   s:(v:string)=>{setBgColor(v);if(fabricRef.current){fabricRef.current.backgroundColor=v;fabricRef.current.renderAll()}}},
          ]).map(c=>(
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
                ...
                <div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                    {filteredLayouts.map(layout=>(
                      <div key={layout.id} className="card-hover"
                        style={{borderRadius:10,overflow:'hidden',border:'1.5px solid #e5e7eb',cursor:'pointer'}}
                        onClick={()=>{
                          // FIX: actually call build() and load onto canvas
                          const fc=fabricRef.current;if(!fc)return
                          const layoutJson=layout.build(cWRef.current,cHRef.current)
                          // save current page, then apply layout to it
                          const saved=[...pagesRef.current]
                          saved[currentPageRef.current]=layoutJson
                          pagesRef.current=saved
                          setPages([...saved])
                          fc.loadFromJSON(layoutJson,()=>{
                            fc.renderAll()
                            captureThumbnail(currentPageRef.current)
                          })
                        }}>
                        <div style={{width:'100%',aspectRatio:'16/10',overflow:'hidden',position:'relative',background:'#f0f0f2'}}>
                ...
                <div>
                  <div style={{display:'flex',gap:6,marginBottom:12}}>
                    <input value={stockQuery} onChange={e=>setStockQuery(e.target.value)}
                      onKeyDown={e=>e.key==='Enter'&&searchStockImages(stockQuery)}
                  ...
                  {stockImages.length===0?(
                    <div style={{textAlign:'center',padding:'20px 0',color:'#9ca3af',fontSize:12}}>
                      ...
                      Search for photos or click a tag
                    </div>
                  ):(
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                      {stockImages.map((url,i)=>(
                        <div key={i} style={{borderRadius:8,overflow:'hidden',cursor:'pointer',border:'2px solid transparent',transition:'all .13s'}}
                          ...
                          <img src={url} alt="" style={{width:'100%',aspectRatio:'16/10',objectFit:'cover',display:'block'}} crossOrigin="anonymous"
                            onError={e=>{(e.target as HTMLImageElement).parentElement!.style.display='none'}}/>
                        </div>
                      ))}
                    </div>
                  )}
                  <div style={{marginTop:12,fontSize:10,color:'#9ca3af',textAlign:'center',fontFamily:'JetBrains Mono,monospace',lineHeight:1.5}}>Photos from Unsplash · Click to add</div>
                </div>
              )}
              ...
              {activePanel==='elements'&&(
                <div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7,marginBottom:16}}>
                    {[
                      {t:'rect',opts:{},icon:<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><rect x="2" y="4" width="16" height="12" rx="2" stroke="currentColor" strokeWidth="1.4"/></svg>},
                      ...
                      {t:'circle',opts:{},icon:<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="7" stroke="currentColor" strokeWidth="1.4"/></svg>},
                      {t:'triangle',opts:{},icon:<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 2L19 18H1L10 2z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>},
                      {t:'line',opts:{},icon:<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M2 18L18 2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>},
                    ].map((s,i)=>(
                      <div key={i} onClick={()=>addShape(s.t,s.opts)}
                        style={{aspectRatio:'1',background:'#f9fafb',borderRadius:8,border:'1.5px solid #e5e7eb',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#6b7280',transition:'all .12s'}}
                  ...
                  {[
                    {label:'Standard (4×3)',  rows:4,cols:3},
                    {label:'Wide (4×5)',       rows:4,cols:5},
                    {label:'Tall (6×3)',       rows:6,cols:3},
                    {label:'Header only (2×3)',rows:2,cols:3},
                  ].map((tbl,i)=>(
                    <div key={i} onClick={()=>addTable(tbl.rows,tbl.cols)}
                      style={{padding:'8px 12px',borderRadius:8,border:'1.5px solid #e5e7eb',marginBottom:7,cursor:'pointer',fontSize:12,fontWeight:500,color:'#374151',transition:'all .12s'}}
              ...
              {activePanel==='text'&&(
                <div>
                  {[
                    ...
                    <div key={i}
                      onClick={()=>addText({text:preset.preview,fs:preset.fs,fw:preset.fw,ff:(preset as any).ff||fontFamily})}
                      ...
                      <div style={{fontSize:Math.min(preset.fs*.48,24),fontWeight:parseInt(preset.fw),fontFamily:`'${(preset as any).ff||fontFamily}',sans-serif`,color:'#0f172a',marginBottom:2}}>{preset.label}</div>
                      <div style={{fontSize:9,color:'#94a3b8',fontFamily:'JetBrains Mono,monospace'}}>{preset.fs}pt · weight {preset.fw}{(preset as any).ff?` · ${(preset as any).ff}`:''}</div>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    {[['#4f46e5','#7c3aed'],['#0ea5e9','#06b6d4'],['#10b981','#34d399'],['#f59e0b','#f97316'],['#ef4444','#ec4899'],['#0f172a','#334155'],['#1e293b','#4f46e5'],['#fce7f3','#ede9fe']].map(([a,b])=>(
                      <div key={a} onClick={()=>{
                        ...
                        fabricRef.current.setBackgroundColor(
                          new fab.Gradient({type:'linear',gradientUnits:'pixels',coords:{x1:0,y1:0,x2:cWRef.current,y2:cHRef.current},colorStops:[{offset:0,color:a},{offset:1,color:b}]}),
                          ()=>fabricRef.current.renderAll()
                        )
                        scheduleAutoSave()
              ...
              {activePanel==='layers'&&(
                <div>
                  ...
                  {fabricRef.current&&[...fabricRef.current.getObjects()].reverse().map((obj:any,i:number)=>{
                    ...
                    const lbl=obj.type==='textbox'||obj.type==='i-text'?`"${(obj.text||'').substring(0,18)}"`:`${obj.type} ${ri+1}`
                    return(
                ...
                <div>
                  ...
                  {[
                    ...
                    <button key={b.label} onClick={b.fn}
                      style={{width:'100%',padding:'12px 14px',borderRadius:10,border:'1.5px solid #e5e7eb',background:'white',cursor:'pointer',display:'flex',alignItems:'center',gap:10,marginBottom:9,textAlign:'left',fontFamily:'Jost,sans-serif',transition:'all .13s'}}
                      onMouseEnter={e=>{e.currentTarget.style.borderColor='#4f46e5';e.currentTarget.style.background='#eef2ff'}}
        ...
        <div style={{width:140,flexShrink:0,background:'white',borderRight:'1px solid #e5e7eb',display:'flex',flexDirection:'column'}}>
          ...
          <div style={{flex:1,overflow:'auto',padding:'8px 7px',display:'flex',flexDirection:'column',gap:8}}>
            {pages.map((page,idx)=>(
              <div key={idx} onClick={()=>switchPage(idx)} className={`pthumb${currentPage===idx?' on':''}`}>
                {/* FIX: show real thumbnail if captured, else solid bg */}
                <div style={{width:'100%',aspectRatio:`${canvasW}/${canvasH}`,background:page?.background??'#ffffff',position:'relative',overflow:'hidden'}}>
                  {thumbnails[idx]
                    ? <img src={thumbnails[idx]} alt="" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>
                    : <div style={{width:'100%',height:'100%',background:typeof page?.background==='string'?page.background:'#ffffff'}}/>
                  }
                  <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'2px 5px',display:'flex',justifyContent:'space-between',alignItems:'center',background:'rgba(255,255,255,.88)'}}>
                    ...
                    {pages.length>1&&(
                      <button onClick={e=>{e.stopPropagation();removePage(idx)}}
                        style={{width:13,height:13,borderRadius:3,background:'#ef4444',border:'none',color:'white',fontSize:8,cursor:'pointer',lineHeight:1}}>×</button>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div style={{padding:'5px 10px',borderTop:'1px solid #e5e7eb',fontSize:10,color:'#94a3b8',fontFamily:'JetBrains Mono,monospace'}}>
            {pages.length}p · {CANVAS_SIZES.find(s=>s.w===canvasW&&s.h===canvasH)?.dims||`${canvasW}×${canvasH}`}
          </div>
        </div>
        ...
        <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'#f0f0f2',overflow:'auto',position:'relative'}}
          ...
          {isDragging&&(
            <div style={{position:'absolute',inset:0,zIndex:50,background:'rgba(79,70,229,.05)',border:'2px dashed #4f46e5',display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
        ...
        {selectedObj&&(
          <div style={{width:192,flexShrink:0,background:'white',borderLeft:'1px solid #e5e7eb',display:'flex',flexDirection:'column'}}>
            ...
            <div style={{padding:14,display:'flex',flexDirection:'column',gap:12,overflow:'auto'}}>
              ...
              {selectedObj.width!=null&&<PRow label="Width"><NIn value={Math.round(selectedObj.width??0)} onChange={v=>updateProp('width',v)}/></PRow>}
              <PRow label="Opacity">
                <div style={{display:'flex',alignItems:'center',gap:7}}>
                  <input type="range" min={0} max={1} step={.01} value={selectedObj.opacity??1}
                    onChange={e=>updateProp('opacity',parseFloat(e.target.value))} style={{flex:1,accentColor:'#4f46e5'}}/>
              ...
              {(selectedObj.type==='textbox'||selectedObj.type==='i-text')&&(
                <PRow label="Font size"><NIn value={selectedObj.fontSize??18} onChange={v=>updateProp('fontSize',v)}/></PRow>
              )}
              {selectedObj.type==='rect'&&(
                <PRow label="Radius"><NIn value={selectedObj.rx??0} onChange={v=>{updateProp('rx',v);updateProp('ry',v)}}/></PRow>
              )}
              <PRow label="Color">
                <input type="color" value={typeof selectedObj.fill==='string'?selectedObj.fill:'#4f46e5'}
                  onChange={e=>updateProp('fill',e.target.value)}
                  style={{width:'100%',height:32,borderRadius:8,border:'1.5px solid #e5e7eb',cursor:'pointer'}}/>
              </PRow>
            </div>
          </div>
        )}
        ...
        <div style={{position:'fixed',inset:0,zIndex:100,background:'rgba(15,23,42,.65)',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(8px)'}}>
          <div style={{background:'white',borderRadius:22,width:'min(980px,96vw)',maxHeight:'92vh',display:'flex',flexDirection:'column',boxShadow:'0 32px 80px rgba(0,0,0,.22)',overflow:'hidden'}}>
            <div style={{padding:'22px 28px',borderBottom:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              ...
              {pages.length>0&&(
                <button onClick={()=>setShowTplModal(false)} style={{background:'#f3f4f6',border:'none',borderRadius:9,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#6b7280'}}>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 3l8 8M11 3l-8 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                </button>
              )}
            </div>
