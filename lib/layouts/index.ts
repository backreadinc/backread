...
export const LAYOUTS: Layout[] = [
  ...
  { id: 'hero-dark-grid', label: 'Dark Grid', cat: 'Hero', preview: '#090C14',
    build: (W, H) => pg('#090C14', [
      ...
      s1.push(...[...Array(8)].map((_, i) => LN(Math.round(W/8*i), 0, Math.round(W/8*i), H, 'rgba(255,255,255,.04)')),)
      s1.push(...[...Array(6)].map((_, i) => LN(0, Math.round(H/6*i), W, Math.round(H/6*i), 'rgba(255,255,255,.04)')),)
  ...
  { id: 'pitch-problem', label: 'The Problem', cat: 'Pitch', preview: '#FFF5F5',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...[['😤', 'Wasted Time', '#FEF2F2', '#991B1B'], ['💸', 'Lost Revenue', '#FFF7ED', '#92400E'], ['🔥', 'Broken Process', '#FFFBEB', '#78350F']].map(([icon, title, bg, col], i) => {)
  ...
  { id: 'pitch-solution', label: 'The Solution', cat: 'Pitch', preview: '#F0FDF4',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...(['#16A34A', '#5B50E8', '#D97706'].map((col, i) => {)
  ...
  { id: 'pitch-market', label: 'Market Size', cat: 'Pitch', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...[['$450B', 'TAM', 'Total Addressable Market', '#5B50E8'], ['$62B', 'SAM', 'Serviceable Addressable Market', '#D97706'], ['$8B', 'SOM', 'Serviceable Obtainable Market', '#16A34A']].map(([val, label, desc, col], i) => {)
  ...
  { id: 'pitch-traction', label: 'Traction', cat: 'Pitch', preview: '#080D1A',
    build: (W, H) => pg('#080D1A', [
      ...
      s1.push(...([['$0M', 'ARR', '#16A34A'], ['0K', 'Customers', '#5B50E8'], ['0%', 'MoM Growth', '#D97706'], ['0', 'NPS', '#EC4899']].map(([val, lbl, col], i) => {)
  ...
  { id: 'pitch-team', label: 'Team', cat: 'Pitch', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...(['CEO & Founder', 'CTO & Co-Founder', 'Head of Growth'].map((role, i) => {)
  ...
  { id: 'pitch-bizmodel', label: 'Business Model', cat: 'Pitch', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...[['Revenue Streams', ['SaaS Subscription', '$99–$499/mo'], '#FFFBEB', '#92400E'], ['Cost Structure', ['R&D · 35%', 'S&M · 40%', 'G&A · 25%'], '#FFF7ED', '#78350F'], ['Unit Economics', ['LTV $12,000', 'CAC $800', 'LTV:CAC 15x'], '#F0FDF4', '#14532D']].map(([title, items, bg, col], i) => {)
        const cw = Math.round((W-140)/3); const cx = 50+i*(cw+20)
  ...
  { id: 'pitch-roadmap', label: 'Roadmap', cat: 'Pitch', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...[['Q1 2025', 'Launch', '#5B50E8'], ['Q2 2025', 'Growth', '#7C3AED'], ['Q3 2025', 'Scale', '#2563EB'], ['Q4 2025', 'Enterprise', '#0F172A']].map(([q, label, col], i) => {)
  ...
  { id: 'prop-exec-summary', label: 'Executive Summary', cat: 'Proposal', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...['Objective 1 — describe the first goal', 'Objective 2 — describe the second goal', 'Objective 3 — describe the third goal'].map((item, i) => TX(`→  ${item}`, { l:Math.round(W*.06), t:Math.round(H*.5)+i*sc(20,W), w:Math.round(W*.8), fs:sc(13,W), fill:'#374151', lh:1.6 })),)
    ])
  },
  { id: 'prop-scope', label: 'Scope of Work', cat: 'Proposal', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...[['Discovery', '1 week'], ['Strategy & Planning', '2 weeks'], ['Design & Build', '6 weeks'], ['Testing & QA', '2 weeks'], ['Launch & Handover', '1 week']].map(([phase, dur], i) => {)
  ...
  { id: 'prop-pricing', label: 'Pricing Table', cat: 'Proposal', preview: '#FFFFFF',
    build: (W, H) => pg('#FFFFFF', [
      ...
      s1.push(...[['Discovery & Strategy', '$2,500'], ['Design & Prototyping', '$5,000'], ['Development', '$12,000'], ['Testing & Launch', '$2,500']].map(([item, price], i) => {)
  ...
  { id: 'prop-timeline', label: 'Project Timeline', cat: 'Proposal', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...['Week 1-2', 'Week 3-5', 'Week 6-9', 'Week 10-12'].map((wk, i) => {)
        const x = Math.round(W*.1)+i*Math.round(W*.22)
  ...
  { id: 'prop-case-study', label: 'Case Study', cat: 'Proposal', preview: '#F0F9FF',
    build: (W, H) => pg('#F0F9FF', [
      ...
      s1.push(...(['↑ 89%', '↑ $2.4M', '↓ 42%', '4.8/5'].map((val, i) => TX(val, { l:Math.round(W*.55), t:Math.round(H*.28)+i*Math.round(H*.13), w:Math.round(W*.36), fs:sc(28,W), fw:'800', fill:'#0369A1', ff:'JetBrains Mono' }))),)
    ])
  },
  ...
  { id: 'editorial-data', label: 'Data Story', cat: 'Editorial', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...[['2.4B', 'People affected globally', '#5B50E8'], ['$847M', 'Industry investment in 2024', '#16A34A'], ['340%', 'Year over year growth rate', '#D97706']].map(([val, desc, col], i) => {)
  ...
  { id: 'stats-kpi-dark', label: 'KPI Dark', cat: 'Stats', preview: '#090C14',
    build: (W, H) => pg('#090C14', [
      ...
      s1.push(...([['↑ 47%', 'Revenue Growth', '#16A34A'], ['↑ 23K', 'New Users', '#5B50E8'], ['94%', 'Retention Rate', '#D97706'], ['4.8★', 'Avg Rating', '#EC4899']].map(([val, lbl, col], i) => {)
        const cw = Math.round((W-120)/4); const cx = 50+i*(cw+13)
  ...
  { id: 'stats-kpi-light', label: 'KPI Light', cat: 'Stats', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...([['$4.2M', 'Monthly Revenue', '#16A34A', '#F0FDF4'], ['↑ 28%', 'Growth MoM', '#5B50E8', '#EEF2FF'], ['8,421', 'Active Users', '#D97706', '#FFFBEB'], ['99.9%', 'Uptime SLA', '#EC4899', '#FDF4FF']].map(([val, lbl, col, bg], i) => {)
  ...
  { id: 'stats-progress', label: 'Progress Metrics', cat: 'Stats', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...([['Revenue Target', 78, '#16A34A'], ['User Acquisition', 62, '#5B50E8'], ['Customer NPS', 91, '#D97706'], ['Product Delivery', 55, '#EC4899']].map(([label, pct, col], i) => {)
  ...
  { id: 'content-three-col', label: '3 Feature Cards', cat: 'Content', preview: '#FFFFFF',
    build: (W, H) => pg('#FFFFFF', [
      ...
      s1.push(...[['⚡', 'Lightning Fast', 'Sub-100ms response times across all core workflows.', '#5B50E8', '#EEF2FF'], ['🔐', 'Enterprise Secure', 'SOC 2 Type II certified with end-to-end encryption.', '#16A34A', '#F0FDF4'], ['📊', 'Real-time Analytics', 'See exactly what happens after you hit send.', '#D97706', '#FFFBEB']].map(([icon, title, body, col, bg], i) => {)
        const cw = Math.round((W-140)/3); const cx = 50+i*(cw+20)
  ...
  { id: 'content-numbered', label: 'Numbered Steps', cat: 'Content', preview: '#FFFFFF',
    build: (W, H) => pg('#FFFFFF', [
      ...
      s1.push(...[['Connect', 'Link your tools in seconds with our native integrations.'], ['Configure', 'Set up your workspace with smart defaults that adapt to you.'], ['Collaborate', 'Invite your team and start working in real time instantly.'], ['Close', 'Hit your goals faster with AI-powered insights and actions.']].map(([title, body], i) => {)
  ...
  { id: 'content-checklist', label: 'Checklist', cat: 'Content', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...['Unlimited document sharing and tracking', 'AI-powered insights for every view', 'Digital signature with legal validity', 'Real-time viewer analytics', 'Forwarding chain detection', 'Custom branding and domain', 'Priority 24/7 support'].map((item, i) => {)
  ...
  { id: 'content-icon-grid', label: 'Icon Grid', cat: 'Content', preview: '#FFFFFF',
    build: (W, H) => pg('#FFFFFF', [
      ...
      s1.push(...([['🚀', 'Fast Setup'], ['🔐', 'Secure'], ['📊', 'Analytics'], ['🤖', 'AI-Powered'], ['🌍', 'Global'], ['💬', '24/7 Support']].map(([icon, label], i) => {)
  ...
  { id: 'team-grid', label: 'Team Grid', cat: 'Team', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...([['Alex Chen', 'CEO'], ['Sarah Kim', 'CTO'], ['Marcus Webb', 'Design'], ['Priya Nair', 'Growth'], ['Daniel Fox', 'Engineering'], ['Luna Park', 'Product']].map(([name, role], i) => {)
  ...
  { id: 'testimonial-triple', label: 'Three Quotes', cat: 'Testimonials', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...(['"Incredible tool that saved us 10 hours per week."', '"The analytics gave us insights we never had before."', '"Best investment we made this year. Full stop."'].map((quote, i) => {)
        const cw = Math.round((W-140)/3); const cx = 50+i*(cw+20)
  ...
  { id: 'testimonial-logos', label: 'Logo Wall', cat: 'Testimonials', preview: '#FAFAFA',
    build: (W, H) => pg('#FAFAFA', [
      ...
      s1.push(...Array.from({length:5}).map((_, i) => {)
  ...
  { id: 'compare-vs', label: 'Us vs Them', cat: 'Compare', preview: '#0F172A',
    build: (W, H) => pg('#0F172A', [
      ...
      s1.push(...['AI-native', 'Real-time', 'No per-seat pricing', 'Setup in 5 min', 'SOC 2 certified'].map((feat, i) => {)
  ...
  { id: 'closing-nextSteps', label: 'Next Steps', cat: 'Closing', preview: '#F8FAFC',
    build: (W, H) => pg('#F8FAFC', [
      ...
      s1.push(...[['📞', 'Schedule a Call', 'Book a 30-min demo with our team', '#5B50E8', '#EEF2FF'], ['📝', 'Sign the Agreement', 'Review and countersign the proposal', '#16A34A', '#F0FDF4'], ['🚀', 'Kick-Off', 'We start within 48 hours of signing', '#D97706', '#FFFBEB']].map(([icon, title, body, col, bg], i) => {)
  ...
  { id: 'closing-contact', label: 'Contact Info', cat: 'Closing', preview: '#0F172A',
    build: (W, H) => pg('#0F172A', [
      ...
      TX('📧  hello@company.com', { l:Math.round(W*.1), t:Math.round(H*.52), w:Math.round(W*.38), fs:sc(13,W), fill:'rgba(255,255,255,.55)', ta:'center', lh:1.6 }),
      TX('🌐  www.company.com', { l:Math.round(W*.52), t:Math.round(H*.52), w:Math.round(W*.38), fs:sc(13,W), fill:'rgba(255,255,255,.55)', ta:'center', lh:1.6 }),
      TX('📱  +1 (555) 000-0000', { l:Math.round(W*.1), t:Math.round(H*.68), w:Math.round(W*.38), fs:sc(13,W), fill:'rgba(255,255,255,.55)', ta:'center', lh:1.6 }),
      TX('📍  San Francisco, CA', { l:Math.round(W*.52), t:Math.round(H*.68), w:Math.round(W*.38), fs:sc(13,W), fill:'rgba(255,255,255,.55)', ta:'center', lh:1.6 }),
    ])
  },
  ...
  { id: 'hero-mono-grid', label: 'Grid Lines', cat: 'Hero', preview: '#FAFAFA',
    build: (W: number, H: number) => pg('#FAFAFA', [
      s1.push(...[...Array(6)].map((_,i) => LN(Math.round(W/6*i), 0, Math.round(W/6*i), H, '#E2E8F0', 1)),)
      s1.push(...[...Array(5)].map((_,i) => LN(0, Math.round(H/5*(i+1)), W, Math.round(H/5*(i+1)), '#E2E8F0', 1)),)
  ...
  { id: 'pitch-financials', label: 'Financials', cat: 'Pitch', preview: '#F8FAFC',
    build: (W: number, H: number) => pg('#F8FAFC', [
      ...
      s1.push(...[['Revenue', '$1.2M', '$2.8M', '$6.5M', '#16A34A'], ['Gross Margin', '42%', '58%', '71%', '#5B50E8'], ['Burn Rate', '$180K/mo', '$240K/mo', '$190K/mo', '#D97706'], ['Runway', '14 mo', '18 mo', '24 mo', '#0369A1']].map(([label, y1, y2, y3, col], i) => {)
        const y = Math.round(H*.28)+i*Math.round(H*.15)
      ...
      s1.push(...[['FY2023','#94A3B8'],['FY2024','#94A3B8'],['FY2025E','#16A34A']].map(([yr,col],i) => TX(yr as string, { l:Math.round(W*.42)+i*Math.round(W*.16), t:Math.round(H*.24), w:100, fs:sc(10,W), fw:'700', fill:col, ta:'center', ff:'JetBrains Mono' })),)
    ])
  },
  { id: 'pitch-gtm', label: 'Go-to-Market', cat: 'Pitch', preview: '#FAFAFA',
    build: (W: number, H: number) => pg('#FAFAFA', [
      ...
      s1.push(...[['Phase 1', 'Direct Sales', 'Target 50 enterprise logos in first 6 months', '0–6 mo', '#5B50E8'], ['Phase 2', 'Channel Partners', 'Build partner network for 10× distribution', '6–18 mo', '#16A34A'], ['Phase 3', 'Product-Led', 'PLG motion with freemium tier + viral loop', '18 mo+', '#D97706']].map(([phase, title, body, time, col], i) => {)
  ...
  { id: 'report-section', label: 'Section Divider', cat: 'Report', preview: '#5B50E8',
    build: (W: number, H: number) => pg('#5B50E8', [
      ...
      s1.push(...Array.from({length:6}).map((_, i) => {)
  ...
  { id: 'pitch-use-cases', label: 'Use Cases', cat: 'Pitch', preview: '#F8FAFC',
    build: (W: number, H: number) => pg('#F8FAFC', [
      ...
      s1.push(...[['💼','Sales Teams','Close deals faster with tracking'],['🏗','Agencies','Impress clients with proposals'],['⚖','Legal','Sign contracts with confidence'],['🚀','Startups','Send pitch decks that convert']].map(([icon,title,body],i) => {)
  ...
  { id: 'report-kpi-page', label: 'KPI Report Page', cat: 'Report', preview: '#FFFFFF',
    build: (W: number, H: number) => pg('#FFFFFF', [
      ...
      s1.push(...[['$4.8M','Revenue','+47%','#16A34A'],['8,421','Users','+23%','#5B50E8'],['94%','Retention','+2pts','#D97706'],['4.8','NPS','+0.3','#EC4899']].map(([val,lbl,chg,col],i) => {)
  ...
  { id: 'infographic-funnel', label: 'Conversion Funnel', cat: 'Infographic', preview: '#F8FAFC',
    build: (W: number, H: number) => pg('#F8FAFC', [
      ...
      s1.push(...[['Visitors','10,000','#EEF2FF','#5B50E8',W*.7],['Leads','3,200','#F0FDF4','#16A34A',W*.55],['Prospects','960','#FFFBEB','#D97706',W*.42],['Customers','240','#FEF2F2','#DC2626',W*.3]].map(([stage,n,bg,col,wPct],i) => {)
